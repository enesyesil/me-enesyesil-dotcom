  ...
  <div class="max-w-6xl mx-auto">
    ...
    {#if allTags.length > 0}
      <div class="flex flex-wrap justify-center gap-2 mb-12 animate-fade-in-up delay-100">
        <button 
          ...
          class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 {selectedTag === 'All' ? 'bg-primary-600 text-white shadow-md' : 'bg-white/50 text-gray-600 hover:bg-white hover:shadow-sm'}"
        >
          All Posts
        </button>
        {#each allTags as tag}
          <button 
            ...
            class="px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 {selectedTag === tag ? 'bg-primary-600 text-white shadow-md' : 'bg-white/50 text-gray-600 hover:bg-white hover:shadow-sm'}"
          >
            {tag}
          </button>
        {/each}
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-fade-in-up delay-200">
      {#each filteredPosts as post}
        <div class="group bg-white/80 backdrop-blur-sm rounded-3xl overflow-hidden shadow-soft hover:shadow-soft-lg hover:-translate-y-1 transition-all duration-300 flex flex-col h-full border border-amber-100/50">
            <!-- Image / Placeholder -->
            ...
            <div class="p-6 flex flex-col flex-grow">
              ...
              <div class="flex flex-wrap gap-2 mb-4">
                 {#if post.tags}
                    {#each post.tags.slice(0, 3) as tag}
                       <span class="px-2 py-1 bg-amber-100 text-amber-800 text-xs rounded-full font-medium">{tag}</span>
                    {/each}
                 {/if}
              </div>
              
              <!-- Title -->
              <h2 class="text-2xl font-bold text-gray-900 mb-3 leading-tight group-hover:text-primary-600 transition-colors">
              ...
              <div class="mt-auto flex items-center justify-between pt-4 border-t border-gray-100">
                 ...
                 <a href="/Blog/{post.slug}" class="inline-flex items-center text-sm font-medium text-primary-600 hover:text-primary-700 group-hover:underline">
                    Read Article <ArrowRightOutline class="w-3.5 h-3.5 ms-1 group-hover:translate-x-1 transition-transform" />
                 </a>
              </div>
            </div>
        </div>
